...
def main():
    ...
    timestamp = rows[0].get("collected_at_utc", "")
    if not timestamp:
        raise RuntimeError("gold_funds.csv has no collection timestamp")

    existing = set()
    if os.path.exists(HISTORY_FILE):
        with open(HISTORY_FILE, "r", encoding="utf-8-sig", newline="") as file:
            for row in csv.DictReader(file):
                existing.add((row.get("collected_at_utc", ""), row.get("symbol", "")))

    new_rows = [
        row for row in rows
        if (row.get("collected_at_utc", ""), row.get("symbol", "")) not in existing
    ]

    if not new_rows:
        print("No new gold-fund history rows to append.")
        return
